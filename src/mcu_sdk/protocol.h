...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "jtlq8aldawnr005a"    //开发平台创建产品后生成的16位字符产品唯一标识
...
//循环定时(可下发可上报)
//备注:/最大10个定时功能: 每10个字节代表； 
///#1通道号（bit0--开关，bit7-bit1---通道号）； 
///#2星期； 
///#3#4起始时间(min)； 
///#5#6结束时间(min)； 
///#7#8开启时间(min)； 
///#9#10关闭时间(min)；
#define DPID_CYCLE_TIME 42
//告警设置(可下发可上报)
//备注:1，告警阈值使能控制及设置1
...
//B，断路器上电时，也上报一次。
//C，面板根据协议下发告警设置。
//5 APP面板支持第一字节能力清单：01短路告警，02浪涌告警，03过载告警，04漏电告警，05高温告警，06打火告警，07功率过高告警，08漏电自检异常告警
#define DPID_ALARM_SET_1 48
//随机定时(可下发可上报)
//备注:/最大16个定时功能：每6个字节代表； 
///#1通道号（bit0--开关，bit7-bit1---通道号----支持多路位数0-6）； 
///#2星期； 
///#3#4起始时间(min)； 
///#5#6结束时间(min)；
#define DPID_RANDOM_TIME 55
//表号(可下发可上报)
//备注:1，设备号 
...
//MCU上报：12343543454
//4，处理逻辑：
//A情形：配网时，上报表号，APP能看到为止，否则需要重新配网。后续每次上报均跟随用水量等DP均执行上报，用于SaaS接收识别管理。
#define DPID_METER_ID 56
//A相功率(只上报)
//备注:
#define DPID_POWER_A 101
//A相电压(只上报)
//备注:
#define DPID_VOLTAGE_A 102
//A相电流(只上报)
//备注:
#define DPID_CURRENT_A 103
//B相功率(只上报)
//备注:
#define DPID_POWER_B 104
//B电压(只上报)
//备注:
#define DPID_VOLTAGE_B 105
//B相电流(只上报)
//备注:
#define DPID_CUREENT_B 106
//C相功率(只上报)
//备注:
#define DPID_POWER_C 107
//C相电压(只上报)
//备注:
#define DPID_VOLTAGE_C 108
//C相电流(只上报)
//备注:
#define DPID_CURRENT_C 109
//今日能耗(只上报)
//备注:
#define DPID_TODAY_ENERGY 110
//当前能耗(只上报)
//备注:
#define DPID_CURENT_ENERGY 111
