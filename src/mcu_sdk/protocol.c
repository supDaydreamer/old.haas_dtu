...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_CYCLE_TIME, DP_TYPE_STRING},
  {DPID_ALARM_SET_1, DP_TYPE_RAW},
  {DPID_RANDOM_TIME, DP_TYPE_STRING},
  {DPID_METER_ID, DP_TYPE_STRING},
  {DPID_POWER_A, DP_TYPE_VALUE},
  {DPID_VOLTAGE_A, DP_TYPE_VALUE},
  {DPID_CURRENT_A, DP_TYPE_VALUE},
  {DPID_POWER_B, DP_TYPE_VALUE},
  {DPID_VOLTAGE_B, DP_TYPE_VALUE},
  {DPID_CUREENT_B, DP_TYPE_VALUE},
  {DPID_POWER_C, DP_TYPE_VALUE},
  {DPID_VOLTAGE_C, DP_TYPE_VALUE},
  {DPID_CURRENT_C, DP_TYPE_VALUE},
  {DPID_TODAY_ENERGY, DP_TYPE_VALUE},
  {DPID_CURENT_ENERGY, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
	...
	printf("=================== upload data start ==========================,%u\r\n",stop_update_flag);
	if (stop_update_flag == ENABLE) return SUCCESS;

    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    //mcu_dp_bool_update(DPID_SWITCH_1,当前开启); //BOOL型数据上报;
#if 0
     length = mcu_dp_value(DPID_ADD_ELE, M_value.dev_power_ele, length);			// 当前电量 dev_power_ele
     length = mcu_dp_value(DPID_CUR_POWER,M_value.dev_power_value, length);		// 当前功率 dev_power_value
     length = mcu_dp_value(DPID_CUR_VOLTAGE, M_value.dev_voltage1, length);		// 电压A
     length = mcu_dp_value(DPID_VOLTAGE_COE, M_value.dev_voltage2, length);		// 电压B
     length = mcu_dp_value(DPID_POWER_COE, M_value.dev_voltage3, length);		// 电压C
     length = mcu_dp_value(DPID_CUR_CURRENT, M_value.dev_current1, length);		// 电流A
     length = mcu_dp_value(DPID_ELECTRIC_COE, M_value.dev_current2, length);		// 电流B
     length = mcu_dp_value(DPID_ELECTRICITY_COE, M_value.dev_current3, length);	// 电流C
#endif

#if 1
    //mcu_dp_bool_update(DPID_SWITCH_1,1); //BOOL型数据上报;
    mcu_dp_value_update(DPID_ADD_ELE,M_value.dev_power_ele); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CUR_CURRENT,M_value.dev_current1); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CUR_POWER,M_value.dev_power_value); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CUR_VOLTAGE,M_value.dev_voltage1); //VALUE型数据上报;
    mcu_dp_value_update(DPID_VOLTAGE_COE,M_value.dev_voltage2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_ELECTRIC_COE,M_value.dev_current2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_POWER_COE,M_value.dev_voltage2); //VALUE型数据上报;
    mcu_dp_value_update(DPID_ELECTRICITY_COE,M_value.dev_current3); //VALUE型数据上报;
#endif
	//mcu_dp_fault_update(DPID_FAULT,当前故障告警); //故障型数据上报;
    //mcu_dp_enum_update(DPID_RELAY_STATUS,当前上电状态); //枚举型数据上报;
    //mcu_dp_string_update(DPID_CYCLE_TIME,当前循环定时指针,当前循环定时数据长度); //STRING型数据上报;
    //mcu_dp_raw_update(DPID_ALARM_SET_1,当前告警设置指针,当前告警设置数据长度); //RAW型数据上报;
    //mcu_dp_string_update(DPID_RANDOM_TIME,当前随机定时指针,当前随机定时数据长度); //STRING型数据上报;
    //mcu_dp_string_update(DPID_METER_ID,当前表号指针,当前表号数据长度); //STRING型数据上报;
    //mcu_dp_value_update(DPID_POWER_A,当前A相功率); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_VOLTAGE_A,当前A相电压); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_CURRENT_A,当前A相电流); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_POWER_B,当前B相功率); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_VOLTAGE_B,当前B电压); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_CUREENT_B,当前B相电流); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_POWER_C,当前C相功率); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_VOLTAGE_C,当前C相电压); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_CURRENT_C,当前C相电流); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_TODAY_ENERGY,当前今日能耗); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_CURENT_ENERGY,当前当前能耗); //VALUE型数据上报;

//	wifi_uart_write_frame(STATE_UPLOAD_CMD, MCU_TX_VER, length);
	printf("=================== upload data end ==========================\r\n");
}
...
/*****************************************************************************
函数名称 : dp_download_voltage_coe_handle
功能描述 : 针对DPID_VOLTAGE_COE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_voltage_coe_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long voltage_coe;
    
    voltage_coe = mcu_get_dp_download_value(value,length);
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_VOLTAGE_COE,voltage_coe);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_electric_coe_handle
功能描述 : 针对DPID_ELECTRIC_COE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_electric_coe_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long electric_coe;
    
    electric_coe = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_ELECTRIC_COE,electric_coe);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_power_coe_handle
功能描述 : 针对DPID_POWER_COE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_power_coe_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long power_coe;
    
    power_coe = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_POWER_COE,power_coe);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_electricity_coe_handle
功能描述 : 针对DPID_ELECTRICITY_COE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_electricity_coe_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long electricity_coe;
    
    electricity_coe = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_ELECTRICITY_COE,electricity_coe);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_cycle_time_handle
功能描述 : 针对DPID_CYCLE_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_cycle_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_string_update(DPID_CYCLE_TIME,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_alarm_set_1_handle
功能描述 : 针对DPID_ALARM_SET_1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_alarm_set_1_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_raw_update(DPID_ALARM_SET_1,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_random_time_handle
功能描述 : 针对DPID_RANDOM_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_random_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_string_update(DPID_RANDOM_TIME,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_meter_id_handle
功能描述 : 针对DPID_METER_ID的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_meter_id_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_string_update(DPID_METER_ID,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_VOLTAGE_COE:
            //电压B处理函数
            ret = dp_download_voltage_coe_handle(value,length);
        break;
        case DPID_ELECTRIC_COE:
            //电流B处理函数
            ret = dp_download_electric_coe_handle(value,length);
        break;
        case DPID_POWER_COE:
            //电压C处理函数
            ret = dp_download_power_coe_handle(value,length);
        break;
        case DPID_ELECTRICITY_COE:
            //电流C处理函数
            ret = dp_download_electricity_coe_handle(value,length);
        break;
        case DPID_RELAY_STATUS:
            //上电状态处理函数
            ret = dp_download_relay_status_handle(value,length);
        break;
        case DPID_CYCLE_TIME:
            //循环定时处理函数
            ret = dp_download_cycle_time_handle(value,length);
        break;
        case DPID_ALARM_SET_1:
            //告警设置处理函数
            ret = dp_download_alarm_set_1_handle(value,length);
        break;
        case DPID_RANDOM_TIME:
            //随机定时处理函数
            ret = dp_download_random_time_handle(value,length);
        break;
        case DPID_METER_ID:
            //表号处理函数
            ret = dp_download_meter_id_handle(value,length);
        break;
...
void mcu_write_rtctime(unsigned char time[])
{
   ...
   */
    if(time[0] == 1) {
		...
		on_haas_time_receive(haas_time);
